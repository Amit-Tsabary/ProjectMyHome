...
# a class that handles all database communication
class UserDatabase:
    ...
    # a method that checks if a certain database exists and if not creates it
    def create_database(self, mydatabase):
        self.mycursor.execute("CREATE DATABASE IF NOT EXISTS " + mydatabase)

    # a method that checks if a certain table exists and if not creates it
...
# a method that checks if login can happen
def login(data):
    username = data[1]
    password = data[2]
...
# a method that resets device status
def reset_status():
    mycursor = UserDatabase.myDb.cursor()
    mycursor.execute("UPDATE devices SET status='off'")


# a method that creates an account with given data
...
# a method that creates a device account with given data
...
# a method that returns all current devices
def get_devices():
    devices = ""
    mycursor = UserDatabase.myDb.cursor()
...
# a method that turns (on/off) a device at a given pin
def turn(status, pin):
    if status == "on":
...
# a method that sends an authentication code to a given email address
...
# a method that checks if the typed code is correct
...
# a method that sets whether the user wants to enable or disable double authentication
...
# a method that returns all current accounts
def get_accounts():
    accounts = ""
    mycursor = UserDatabase.myDb.cursor()
...
# a method that returns all current rooms
def get_rooms():
    rooms = ""
    mycursor = UserDatabase.myDb.cursor()
...
# a method that returns all devices with id's that are in the given list
...
# a method that edits a given room
...
# a method that turns (on/off) all devices with the given id's
...
# a method that deletes a device with the given id
...
# a method that deletes a room with the given id
...
# a method that deletes an account with the given id
...
# a method that edits a given account
...
# a class that handles the requests from the user and sends the corresponding response
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path.__contains__("/turnOn_"):
            ...
            data = self.path.split("_")
            delete_room(data[1])
        elif self.path.__contains__("deleteAccount_"):
            data = self.path.split("_")
            delete_account(data[1])
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Please check arduino board connection and try again")
    else:
        reset_status()
